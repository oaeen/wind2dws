...
class WindNet_ERA5_G(nn.Module):
    def __init__(self, input_channels=2, output_size=1):
        super(WindNet_ERA5_G, self).__init__()
        ...
        self.out_conv = nn.Conv2d(10, 1, kernel_size=1)

    def forward(self, g, l=None):
        ...
        x = x.permute(1, 0, 2)
        x = x.view(batch_size, g_sample_days, 30, 24)
        x = self.out_conv(x)
        x = torch.squeeze(x)
...
class Wind_Feature_Head(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size, 720, self.out_conv_in)
        x = x.permute(0, 2, 1)
        x = self.out_conv(x)
        x = torch.squeeze(x)
