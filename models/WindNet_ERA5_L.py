...
# 对于 ERA5 只输入 local 风场数据
class WindNet_ERA5_L(nn.Module):
    def __init__(self, input_channels=2, output_size=1):
        super(WindNet_ERA5_L, self).__init__()
    ...
    def forward(self, _g, l):
        ...
        x = x.permute(1, 0, 2)
        x = x.view(batch_size, l_sample_days, 30, 24)
        x = self.out_conv(x)
        x = torch.squeeze(x)
